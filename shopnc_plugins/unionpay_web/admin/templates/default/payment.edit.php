  ...
  <form id="post_form" method="post" name="form1">
    ...
    <table class="table tb-type2 nobdb">
      <tbody>
        ...
        <?php } elseif ($output['payment']['payment_code'] == 'unionpay') { ?>
        <tr>
            <td colspan="2" class="required">银联商户号: </td>
        </tr>
        <tr class="noborder"><input type="hidden" name="config_name" value="unionpay_account,cert_passwd,bank_state" />
            <td class="vatop rowform"><input name="unionpay_account" id="unionpay_account" value="<?php echo $output['config_array']['unionpay_account'];?>" class="txt" type="text"></td>
            <td class="vatop tips"></td>
        </tr>
        <tr>
            <td colspan="2" class="required">签名证书密码: </td>
        </tr>
        <tr class="noborder">
            <td class="vatop rowform"><input name="cert_passwd" id="cert_passwd" value="<?php echo $output['config_array']['cert_passwd'];?>" class="txt" type="text"></td>
            <td class="vatop tips"></td>
        </tr>
        <tr>
            <td colspan="2" class="required">启用网银直连支付:（修改显示银行：\shop\api\payment\unionpay\banklist.php） </td>
        </tr>
        <tr class="noborder">
            <td class="vatop rowform onoff"><label for="bank_state1" class="cb-enable <?php if($output['config_array']['bank_state'] == '1'){ ?>selected<?php } ?>" ><span><?php echo $lang['nc_yes'];?></span></label>
                <label for="bank_state2" class="cb-disable <?php if($output['config_array']['bank_state'] == '0'){ ?>selected<?php } ?>" ><span><?php echo $lang['nc_no'];?></span></label>
                <input type="radio" <?php if($output['config_array']['bank_state'] == '1'){ ?>checked="checked"<?php }?> value="1" name="bank_state" id="bank_state1">
                <input type="radio" <?php if($output['config_array']['bank_state'] == '0'){ ?>checked="checked"<?php }?> value="0" name="bank_state" id="bank_state2"></td>
            <td class="vatop tips"></td>
        </tr>
        <?php } ?>
        <tr>
