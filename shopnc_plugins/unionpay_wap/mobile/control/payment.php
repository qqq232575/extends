...
class paymentControl extends mobileHomeControl{
     ...
     * 银联支付支付回调
     */
    public function return_unionpayOp() {
     ...
     * 银联手机支付提醒
     */
    public function notify_unionpayOp() {
        ...
        $mb_payment_info = $this->_get_payment_config();
        if(!$mb_payment_info) {
            output_error('系统不支持选定的支付方式');
        }

        $payment_api = new $this->payment_code($mb_payment_info);
